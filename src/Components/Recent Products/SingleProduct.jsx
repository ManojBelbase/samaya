      ...
      <div className="grid grid-cols-1 gap-4 rounded-lg border px-3 py-5 md:grid-cols-2">
        <div className="max-h-96 overflow-hidden">
          <img
            src={product.image}
            alt={product.name}
            className="h-full w-full transform cursor-pointer object-cover transition-transform duration-300 ease-in-out hover:scale-150"
          />
        </div>
        <div className="details text-white">
          <h2 className="mb-2 text-4xl font-semibold">{product.name}</h2>
          <p className="mb-2">{product.description}</p>
          <p className="mb-2">Brand: {product.brand.brandName}</p>
